...
const validatePaymentAmounts = () => {
    ...
    for (let i = 0; i < props.payables.length; i++) {
        ...
        if (
            ...
            parseFloat(payable.remaining_amount)
        ) {
...
const handleSubmit = () => {
    ...
    const payload = {
        payable_ids: props.payables.map((r) => r.id),
    ...
    axios
        .post(route("payables.payments.store"), payload)
        .then(() => {
            loading.value = false;
            window.location.href = route("payables.payments.index");
        })
        .catch((error) => {
            loading.value = false;
